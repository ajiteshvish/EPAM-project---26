...
export default function TransactionsPage() {
  ...
  const filtered = filterTransactions(transactions, { search, type, category, sortBy });
  const handleExportCSV = () => {
    const headers = ["ID", "Title", "Category", "Type", "Amount", "Date", "Status", "Reference"];
    const rows = filtered.map((t) => [t.id, `"${t.title}"`, t.category, t.type, t.amount, t.date, t.status, `"${t.reference || ""}"`]);
  ...
  return (
    <div className="space-y-6 font-mono">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b-[3px] border-[#0a0a0a] pb-4">
        <div><h2 className="font-grotesk font-black text-[24px] uppercase text-[#0a0a0a]">Audit Ledger ({filtered.length} Entries)</h2><p className="text-[11px] uppercase tracking-[0.14em] text-[#0a0a0a]/60 mt-0.5">Complete real-time transaction ledger &amp; search records</p></div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#f4f1ea] border-[3px] border-[#0a0a0a] p-4 shadow-hard-sm">
        ...
        <TransactionFilters type={type} onTypeChange={setType} category={category} onCategoryChange={setCategory} sortBy={sortBy} onSortChange={setSortBy} />
      </div>
